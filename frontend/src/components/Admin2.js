...
const Admin2 = () => {
    ...
    const fetchAirportsByCountry = async (countryName) => {
        ...
        setError(null);
        try {
            const response = await fetch(`http://localhost:5000/api/airports/country/${countryName}`);
            if (!response.ok) {
    ...
    const handleDeleteAirport = async (airportCity) => {
        try {
            const response = await fetch(`http://localhost:5000/api/airports/${airportCity}`, {
                method: 'DELETE'
            });
