...
export default function Admin1_1() {
    ...
    const handleViewFlights = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/flightsbyDate?startDate=${viewDate}`);
            if (!response.ok) {
    ...
    const handleDeleteFlight = async (flightNumber) => {
        try {
            const response = await fetch(`http://localhost:5000/api/flights/cancel/${flightNumber}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
